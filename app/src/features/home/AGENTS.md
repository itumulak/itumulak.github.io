...
- The `#blog` section in `index.astro` previews the `RECENT_POSTS_COUNT` (3, a local default; no spec pins this number) most recent posts via the blog feature's `getPublishedPosts()` (`../blog/posts.ts`), never `getCollection('blog')` directly, per that feature's own convention. Post links point at the `/blog/[slug]/` route, built in `../../pages/blog/[slug].astro` (spec 0004).
